"""
AAA2026 unified updater: incremental fetch → score → profile gen.

Designed to be run periodically (every ~30 min via cron or launchd) to keep
the aaa2026 data fresh end-to-end.

Pipeline:
  1. Fetch new AAA games (yesterday + today by default) via gamefeed JSON
  2. Append non-dup pitches to pitches_aaa2026
  3. Re-score the affected pitches via the model and UPDATE
     pitches_aaa2026_scored with new stuff_plus values
  4. Aggregate scored table into profiles/output/aaa2026/json/

Run:
  python update_aaa2026.py             # incremental (yesterday + today)
  python update_aaa2026.py --full      # full season rebuild (slow)
"""
...
TABLE = "pitches_aaa2026"
TABLE_SCORED = "pitches_aaa2026_scored"
PROFILE_DIR = "/Users/akane/Desktop/new_stuff/stuff_plus/profiles/output/aaa2026/json"
...
def step_score():
    ...
    # (e.g. VAA_adj vs vaa_adj) so a fresh CREATE TABLE doesn't collide.
    seen, keep = {}, []
...
def step_profiles():
    """Aggregate from TABLE_SCORED → profile JSONs in profiles/output/aaa2026/json/."""
    ...
    generate_all_cards(df, season="aaa2026", skip_png=True)
